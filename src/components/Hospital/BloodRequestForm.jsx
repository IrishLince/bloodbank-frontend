...
const BloodRequestForm = () => {
  ...
  const [activeCalendar, setActiveCalendar] = useState('request'); // 'request' or 'needed'
  ...
  // Mock data for blood admin centers (from NewRequest.jsx)
  ...
  const handleConfirmSubmit = () => {
    ...
    // Filter out empty blood requests before passing to success page
    const validBloodRequests = bloodRequests.filter(request => 
      request.bloodType && request.unitsRequested
    );
    
    // Create request object for caching
    const requestData = {
      id: `REQ-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000).toString().padStart(3, "0")}`,
    ...
    // Store in browser cache (localStorage)
    ...
    // Update blood center inventory (simulate real-time update)
    const updatedCenters = bloodAdminCenters.map(center => {
      if (center.id === selectedAdminId) {
        const updatedInventory = center.inventory.map(item => {
          const requestedUnits = validBloodRequests.find(req => req.bloodType === item.type)?.unitsRequested || 0;
          return {
            ...item,
            available: Math.max(0, item.available - parseInt(requestedUnits))
          };
        });
        return { ...center, inventory: updatedInventory };
      }
      return center;
    });
    
    // Update localStorage with new inventory
    localStorage.setItem('bloodAdminCenters', JSON.stringify(updatedCenters));
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="space-y-3">
                  <div className="space-y-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveCalendar('request')}
              ...
              <div>
                ...
                <ul className="list-disc list-inside space-y-1 text-red-700">
                  {Object.entries(errors)
                    .filter(([key]) => !key.includes('_') && key !== 'duplicateBloodTypes' && key !== 'maxRequests')
                    .map(([key, error], index) => (
